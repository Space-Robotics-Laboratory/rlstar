...
class PPMCRLX():
    ...
    def initiate(self):
        """
        initiates the scene by moving markers to indicate the waypoint locations
        :return:
        """
        if self.isFirst:
    ...
    def getState(self):
        ...
        This is the information that the RL agent has to learn from. Values are normalized to fit [-1,1]
        :return:
        """
    ...
    def getDistance2Waypoint(self, xyz):
        """
        gets the distance between the robot and the current waypoint. Saves the old waypoint distance for future comparison.
        :return:
        """
    ...
    def getReward(self):
        """
        calculates the reward given the state observation
        the main reward is given for advancing towards the waypoint. Constants are stored in the constants file.
        several penalties have been tried to tweak the behavior of the rover. However, it also works with a simple reward given only the
        main reward and a time penalty
        :return:
        """
    ...
    def getVelocity2Waypoint(self):
        """
        Gets the advancement of the rover towards the waypoint. Can be either positive or negative. If a waypoint is reached
        a bonus is given to the rover instead of the normal value
        :return:
        """
        goalsLeft = const.numGoals - self.goalCounter
        if goalsLeft <= 0:
    ...
    def check_boundry(self):
        """
        this function checks whether the robot is within the boundary around the waypoint. If it is within this boundary, it has successfully reached the waypoint
        dis_threshold is the boundary
        :return:
        """
    ...
    def create_randomGoals(self, numGoals=2, center=0, width=10, length=10, minDif=3, boundry=4):
        """
        creates random goals for the rover. currently designed for 2 goals with a square region of 400m squared, however, should be easy to expand
        :param numGoals: number of waypoints per episode
        :param center: robot starting point
        :param width: width of the map
        :param length: length of the map
        :param minDif: minimum distance difference between any two goals (gets hard to initialize if there are many and mindif is big)
        :return:
        """
    ...
    def getAction(self, action):
        """
        get the action from the main file in order to print it
        """
        self.action = action

    def print(self):
        """
        this file is used for controlling what to print and at what frequency
        :return:
        """
        printStateFreq = 5
        printRewardFreq = 15
    ...
    def isDoneX(self):
        """
        This functon checks the robot against fail conditions. It will print information used to track the progress
            Inputs:
                None
            Outputs:
